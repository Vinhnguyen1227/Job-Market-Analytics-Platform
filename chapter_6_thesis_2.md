...
### 6.4.1 Location and Geographic Parsing
...
### 6.4.2 Unstructured Salary Categorization
...
This parser identifies numeric ranges, ignores foreign currencies via specific exclusion patterns, standardizes values into millions of VND, and mathematically evaluates the bounds (`[lo, hi]`) against predefined salary tiers. This transforms free text into indexable categorizations like "10 – 20 triệu" without requiring a backend round-trip.

### 6.4.3 Experience Tier Mapping
Similarly, the `getExpBuckets` parser processes strings like "Dưới 1 năm" or "2-5 tháng". It converts all parsed numeric values into a standardized yearly metric and determines intersection overlaps with predefined experience tiers (e.g., "1 – 2 năm"). By executing these deterministic parsers directly in the browser's JavaScript engine, the frontend remains highly responsive while ensuring data conforms strictly to the schema expected by the Elasticsearch backend.
