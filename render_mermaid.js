...
if (!fs.existsSync('thesis_2_latex/images')) {
    fs.mkdirSync('thesis_2_latex/images', { recursive: true });
}
...
files.forEach(file => {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        const outName = `thesis_2_latex/images/${chapterName}_diagram_${counter}.pdf`;
        
        fs.writeFileSync(tempName, diagramContent);
        console.log(`Rendering ${tempName} to ${outName}...`);
        try {
            execSync(`npx -y @mermaid-js/mermaid-cli -i ${tempName} -o ${outName} -p puppeteer-config.json`, { stdio: 'inherit' });
        } catch (e) {
            console.error(`Failed to render ${tempName}`);
            // Sometimes it requires ignoring sandbox for puppeteer
            try {
                execSync(`npx -y @mermaid-js/mermaid-cli -i ${tempName} -o ${outName} -p puppeteer-config.json`, { stdio: 'inherit' });
            } catch (e2) {
