...
const files = fs.readdirSync(__dirname).filter(f => f.match(/chapter_\d+_thesis_1\.md$/));

if (!fs.existsSync('thesis_1_images')) {
    fs.mkdirSync('thesis_1_images', { recursive: true });
}
...
files.forEach(file => {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        const outName = `thesis_1_images/${chapterName}_diagram_${counter}.png`;
