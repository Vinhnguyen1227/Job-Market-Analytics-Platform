...
async def dispatch(tc, session_id: str, session_data: dict | None, user_message: str) -> dict:
    ...
    if tool == "assess_resume":
        ...
        md = await adapter_mgr.generate(
            "hr_coach", system=HR_COACH_SYSTEM_PROMPT, user=user_prompt,
        )
        return {
            "response": md,
            "task_type": tool,
    ...
    if tool == "interview_prep":
        ...
        md = await adapter_mgr.generate(
            "structured_gen", system=system_prompt, user=user_prompt,
        )
        return {
            "response": md,
            "task_type": tool,
    ...
    if tool == "general_response":
        md = await adapter_mgr.generate(
            "hr_coach", system=HR_COACH_SYSTEM_PROMPT, user=user_message,
        )
        return {
            "response": md,
            "task_type": tool,
